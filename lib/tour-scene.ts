...
export function createRenderedTour(host:HTMLDivElement,onRoom:(id:string)=>void,onReady:()=>void,onError:(message:string)=>void){
 ...
 renderer.domElement.tabIndex=0;renderer.domElement.setAttribute('aria-label','Virtual apartment tour. Drag to look, scroll to zoom, click a room marker to move.');host.appendChild(renderer.domElement);
 ...
 async function jump(id:string){pendingRoom=id;if(!manifest)return;const view=defaultView(id);if(!view)return;const token=++request;status.textContent='Opening this room…';status.hidden=false;
  try{const tex=await textureLoader.loadAsync(`/daylight-tour/${view.id}.jpg`);if(disposed||token!==request){tex.dispose();return;}
   ...
   for(const next of view.links){const dest=defaultView(next);if(!dest)continue;const anchor=manifest!.anchors[view.id+':'+next]||dest.position;
    ...
    const button=document.createElement('button');button.className='tour-hotspot';button.type='button';const label=dest.name.replace(' · inferred','');button.textContent='↗ '+label;button.setAttribute('aria-label','Move to '+label);button.onclick=()=>void jump(next);overlay.appendChild(button);pins.push({button,direction});
   }
   status.hidden=true;onRoom(view.room);onReady();
  }catch{if(!disposed&&token===request){status.hidden=true;onError('This room could not load. Please try again or choose another room.');}}
 }
 fetch('/daylight-tour/manifest.json').then(r=>{if(!r.ok)throw new Error('Manifest unavailable');return r.json() as Promise<Manifest>;}).then((m:Manifest)=>{if(disposed)return;manifest=m;void jump(pendingRoom);}).catch(()=>{if(!disposed)onError('The tour could not load. Please try again or explore the original photos.');});
 ...
 const forward=new THREE.Vector3();camera.getWorldDirection(forward);for(const pin of pins){const visible=pin.direction.dot(forward)>0;const p=pin.direction.clone().project(camera);pin.button.hidden=!visible||Math.abs(p.x)>.92||Math.abs(p.y)>.83;const half=pin.button.offsetWidth/2;const x=THREE.MathUtils.clamp((p.x*.5+.5)*host.clientWidth,half+12,host.clientWidth-half-12);pin.button.style.left=`${x}px`;pin.button.style.top=`${(-p.y*.5+.5)*100}%`;}
